...
export const WSSIOrb: React.FC<WSSIOrbProps> = ({{
  data,
  width = 800,
  height = 600,
  className = '',
  ...
  const getCoreColor = (value: number): string => {{
    if (value < -1.5) return '#ff0040'; // Critical - red
    if (value < -1.0) return '#ff8800'; // Approaching - amber
    if (value < -0.5) return '#ffcc00'; // Watch - yellow
    return '#00ffcc'; // Stable - cyan
  }};
